function attachEvents() {
    ...
    let weatherBttn = document.querySelector("#submit");
    ...
    weatherBttn.addEventListener("click", async () => {
        try {
            ...
            let response2 = await fetch(`${url}today/${neededLocation.code}`);
            let forecastForToday = await response2.json();

            let response3 = await fetch(`${url}upcoming/${neededLocation.code}`);
